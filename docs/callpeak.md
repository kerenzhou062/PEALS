...
PEALS will save all output files into the specified folder for this
...
Also, PEALS can detect and read gzipped file. For example, `.bed.gz`
...
`BAMPE` or `BEDPE`. In this way, PEALS will process the `BAM` or `BED`
files as paired-end data. Instead of building a bimodal distribution
of plus and minus strand reads to predict fragment size, PEALS will
...
standard `BED` format. You can use PEALS subcommand `randsample` to
convert a `BAM` file containing paired-end information to a `BEDPE`
format file:

```
PEALS randsample -i the_BAMPE_file.bam -f BAMPE -p 100 -o the_BEDPE_file.bed
```
...
The p-value cutoff. If `-p` is specified, PEALS will use p-value instead
of q-value.
...
merged as one. If they are not set, PEALS will set the DEFAULT value
